...
SELECT
    ...
    (SELECT COUNT(*) FROM "COURSE_BOOKING" WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明') AND status not in ('課程已取消')) AS remaining_credit
    FROM "USER"
    WHERE name = '王小明';
...
SELECT
    ...
    FROM "COURSE_BOOKING"
    WHERE status = '即將授課' AND booking_at BETWEEN '2024-11-01' AND '2024-11-30';
